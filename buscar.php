...
<title>Buscar &#8211; <?php echo $title;?></title>
    ...
    <div class="page-wrapper">
      ...
      <div class="de-page-title uk-section" data-layout="trina" data-render-location="page" style="background-color:#5d1030; background-image:url(images/gestion.jpg);background-position:left top;background-repeat:no-repeat;background-size:cover;background-attachment:scroll;">
        <div class="de-page-title__container uk-container">
          <h1 class=" uk-heading-primary" style="font-size: 49px;">Buscando: <?php echo $s;?></h1>
          <ul class="uk-breadcrumb uk-text-small">
            <li> <a class="uk-link" href="./" title="<?php $title;?>"> Inicio </a> </li>
            <li> <span class="bread-current"> Buscando: <?php echo $s;?> </span> </li>
          </ul>
        </div>
      </div>
...
<?php
$buscar = $_GET['s'];
$query_Search = "SELECT titulo, categoria, dia, mes, anio, contenido FROM (SELECT titulo, categoria, dia, mes, anio, contenido FROM blog UNION SELECT titulo, categoria, dia, mes, anio, contenido FROM testimonios UNION SELECT titulo, categoria, dia, mes, anio, contenido FROM videos ) AS tablas WHERE tablas.titulo LIKE '%$buscar%'";
$resultado_Search = mysql_query($query_Search);
$contador = 1;
while($row_Search = mysql_fetch_array($resultado_Search))
{
if ($contador%2==0){
    $la_clase = "";
...
?>
			  ...
			  <div class="<?php echo $la_clase;?>">
                <article id="post-<?php echo $row_Search['id'];?>" class="uk-article uk-margin-medium post type-post status-publish format-standard has-post-thumbnail hentry category-social"> <!--<img width="1900" height="1268" class="img-responsive de-img-thumbnail wp-post-image ds-lazy-image lazy-loaded" alt="blog-01-min" sizes="(max-width: 1900px) 100vw, 1900px" data-uk-img="" data-src="<?php echo $row_Search['imagen'];?>" data-srcset="<?php echo $row_Search['imagen'];?> 1900w, <?php echo $row_Search['imagen'];?> 1000w, <?php echo $row_Search['imagen'];?> 300w, <?php echo $row_Search['imagen'];?> 768w, <?php echo $row_Search['imagen'];?> 1024w" src="<?php echo $row_Search['imagen'];?>" srcset="<?php echo $row_Search['imagen'];?> 1900w, <?php echo $row_Search['imagen'];?> 1000w, <?php echo $row_Search['imagen'];?> 300w, <?php echo $row_Search['imagen'];?> 768w, <?php echo $row_Search['imagen'];?> 1024w">-->
                  ...
                  <div class="de-archive__wrapper uk-margin-medium uk-position-relative">
                    ...
                    <div class="entry-meta">
                      <ul class="uk-text-small uk-subnav uk-subnav-divider uk-margin de-post-meta" data-uk-margin="">
                        <li class="uk-width-auto uk-first-column"><?php echo $row_Search['mes'];?> <?php echo $row_Search['dia'];?>, <?php echo $row_Search['anio'];?></li>
                        <li class="uk-width-auto">
                          <div class="uk-flex uk-width-1 de-post-meta__categories"><?php echo $row_Search['categoria'];?></div>
                        </li>
                      </ul>
                    </div>
                    <div class="entry-title">
                      <h2 class="uk-margin-remove uk-h3"><?php echo $row_Search['titulo'];?></h2>
                    </div>
                  </div>
                  <div class="de-archive__featured-content uk-margin-medium">
                    <p class="uk-margin-remove"> 
						<?php echo $row_Search['contenido'];?>
					  </p>
					  
					<?php if($row_Search['categoria'] == 'Blog Sitio'){?>  
                    <a class="uk-margin-medium-top uk-button uk-button-secondary" href="noticias/<?php echo $row_Search['permalink'];?>">Continua leyendo</a>
					<?php } else {?>
					  <a class="uk-margin-medium-top uk-button uk-button-secondary" href="https://www.facebook.com/MontserratCaballero.Bc/<?php echo $row_Search['permalink'];?>" target="_blank">Ver en Facebook</a>
					<?php }?>
